...
export function useLeads(filter: LeadFilter) {
  return useQuery<PagedResult<LeadSummary>>({
    ...
    queryFn: async () => {
      ...
      if (filter.hasPhone !== undefined) params.set('hasPhone', String(filter.hasPhone))
      if (filter.hasMobilePhone !== undefined) params.set('hasMobilePhone', String(filter.hasMobilePhone))
      if (filter.hasContact !== undefined) params.set('hasContact', String(filter.hasContact))
